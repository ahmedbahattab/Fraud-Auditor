...
def View_Prediction_Of_Fraud_In_Health_Insurance_Ratio(request):
    detection_ratio.objects.all().delete()
    ratio = ""
    kword = 'No Fraud Found'
    print(kword)
    ...
    return render(request, 'SProvider/View_Prediction_Of_Fraud_In_Health_Insurance_Ratio.html', {'objs': obj})
...
def Download_Predicted_DataSets(request):
    ...
    for my_row in data:
        ...
        ws.write(row_num, 1, my_row.ClaimID, font_style)
        ws.write(row_num, 2, my_row.ClaimDt, font_style)
        ws.write(row_num, 3, my_row.Provider, font_style)
        ws.write(row_num, 4, my_row.Sun_Insured, font_style)
        ws.write(row_num, 5, my_row.InscClaimAmtReimbursed, font_style)
        ws.write(row_num, 6, my_row.AttendingPhysician, font_style)
        ws.write(row_num, 7, my_row.ClmDiagnosisCode_1, font_style)
        ws.write(row_num, 8, my_row.Claimed_Amount, font_style)
        ws.write(row_num, 9, my_row.Glucose, font_style)
        ws.write(row_num, 10, my_row.BloodPressure, font_style)
        ws.write(row_num, 11, my_row.SkinThickness, font_style)
        ws.write(row_num, 12, my_row.Insulin, font_style)
        ws.write(row_num, 13, my_row.BMI, font_style)
        ws.write(row_num, 14, my_row.DiabetesPedigreeFunction, font_style)
        ws.write(row_num, 15, my_row.Age, font_style)
...
def train_model(request):
    ...
    detection_accuracy.objects.create(names="Graph Neural Network-GNN",
